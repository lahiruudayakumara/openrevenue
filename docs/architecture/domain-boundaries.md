...
All application ports require the canonical tenant, jurisdiction, actor, and
correlation context described in the
[foundational domain model](foundational-domain-model.md). These shared values
are the bounded contexts' narrow shared kernel; ownership of taxpayer identity,
tax policy, financial facts, and workflow remains with the contexts above.
